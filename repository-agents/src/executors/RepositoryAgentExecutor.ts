...
import type { BaseAgent, BaseAgentConfig } from '@developer-agent/shared';
...
/**
 * AgentExecutor implementation for Repository Agents.
 * Wraps any BaseAgent subclass (Node.js, React, Angular, C#, etc.) and adapts it to the @a2a-js execution model.
 * This is a generic executor that works with all repository agent types.
 */
export class RepositoryAgentExecutor implements AgentExecutor {
  ...
  /**
   * Factory method to create a RepositoryAgentExecutor with any agent implementation.
   * @param agentClass The BaseAgent subclass to instantiate
   * @param config Configuration for the agent
   */
  static create<T extends BaseAgent>(
    agentClass: new (repositoryName: string, config?: Partial<BaseAgentConfig>) => T,
    repositoryName: string,
    config?: Partial<BaseAgentConfig>
  ): RepositoryAgentExecutor {
    const agent = new agentClass(repositoryName, config);
    return new RepositoryAgentExecutor(agent);
  }
