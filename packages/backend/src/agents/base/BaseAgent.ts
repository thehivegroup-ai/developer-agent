...
export abstract class BaseAgent implements IAgent {
  ...
  protected messageHandlers: Map<
    string,
    (message: AgentMessage) => Promise<AgentMessage>
  > = new Map();
  ...
  extendTTL(additionalMinutes: number): void {
    this.ttlExpiresAt = new Date(
      this.ttlExpiresAt.getTime() + additionalMinutes * 60 * 1000
    );
    this.lastActivityAt = new Date();
  }
  ...
  async handleMessage(message: AgentMessage): Promise<AgentMessage> {
    ...
    if (!action) {
      return this.createErrorResponse(
        message,
        'NO_ACTION',
        'Message does not contain an action'
      );
    }
