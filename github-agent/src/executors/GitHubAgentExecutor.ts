...
/**
 * AgentExecutor implementation for the GitHub Agent.
 * Wraps BaseGitHubAgent and adapts it to the @a2a-js execution model.
 */
export class GitHubAgentExecutor implements AgentExecutor {
  private readonly agent: BaseGitHubAgent;
  private readonly runningTasks: Map<string, AbortController> = new Map();

  constructor(agent: BaseGitHubAgent) {
    this.agent = agent;
  }

  /**
   * Factory method to create a GitHubAgentExecutor with a concrete agent implementation.
   * @param agentClass The BaseGitHubAgent subclass to instantiate
   * @param config Configuration for the agent
   */
  static create<T extends BaseGitHubAgent>(
    agentClass: new (config?: Omit<BaseAgentConfig, 'agentType'>) => T,
    config?: Omit<BaseAgentConfig, 'agentType'>
  ): GitHubAgentExecutor {
    const agent = new agentClass(config);
    return new GitHubAgentExecutor(agent);
  }
