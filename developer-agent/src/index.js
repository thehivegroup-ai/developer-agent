...
import { MessageRouter, MessagePersistence, CheckpointManager, } from '@developer-agent/shared';
// Developer Agent
...
export class DeveloperAgent extends BaseDeveloperAgent {
    ...
    async shutdown() {
        ...
        if (this.state) {
            await this.checkpointManager.saveCheckpoint(this.state.sessionId, this.state, 'completed');
        }
     ...
     */
    async processQuery(query, userId, threadId) {
        ...
        try {
            ...
            if (this.state) {
                this.state = StateManager.updateStatus(this.state, 'failed', error instanceof Error ? error.message : 'Unknown error');
    ...
    setupRouterListeners() {
        this.router.on('message:enqueued', (message) => {
